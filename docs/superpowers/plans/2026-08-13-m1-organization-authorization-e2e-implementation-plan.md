...
- Preserve deny-by-default Spring Security. Starting in Task 5, open each completed M1 operation only with its exact `HttpMethod` and exact path pattern as authenticated; never introduce broad `/users/**`, `/projects/**`, `/teams/**`, or equivalent family matchers, even between tasks. Every route-delivering task must add `SecurityConfigurationTest` coverage proving the implemented pair requires authentication, an unsupported method on the same path is denied, and final `anyRequest().denyAll()` remains effective. All M2 routes remain denied.
...
| 6 | `roleScopeMatrixControlsAssignmentCreation`, `sensitiveMutationBypassesWarmContextCache`, `foreignTypedScopeIsNotFound`, `duplicateAssignmentIsConflict`, `roleRevokeBumpsVersionAndAudits`, `statusChangeBumpsVersionAndAudits`, `staleExpectedVersionIsConflict`, `staleSameStatusIsConflict`, `matchingSameStatusIsNoOp` |
...
| 16 | `openApiParses`, `openApiContainsExactlyApprovedM1Operations`, `pagedContractsUseExistingPageShape`, `catalogContractsAreUnpaged`, `idsAndVersionsAreSerializedAsStrings`, `problemResponsesUseProblemSchema`, `eachM1OperationDeclaresExactResponseStatuses` |
...
| 18 | `usersCoversLoadingEmptyErrorAndData`, `userActionsRespectIndependentPermissions`, `userStatusSendsExpectedVersion`, `userStatusConflictRefetchesWithoutRetry`, `userMutationInvalidatesQueries`, `rolesCatalogIsReadOnlyAndUnpaged` |
...
**Security decision checkpoint:** Treat MySQL-first bearer security-version validation as an intentional M1 security-over-latency tradeoff: Redis recovery cannot resurrect an old JWT; the 60-second Redis authorization-context cache remains unchanged; `app_user.security_version` is durable truth; after-commit cleanup reuses `RedisRefreshSessionRepository.revokeAll(long userId)`; do not add distributed invalidation machinery. Defer performance benchmarking until after V1 correctness.
...
- [ ] **Step 1: Write RED API tests.** Add `usersListAndDetailShareFullRepresentation`, `usersAreCurrentOrgOnly`, `usersCountMatchesRowPredicate`, `rolesAndPermissionsAreUnpaged`, `missingReadPermissionIsForbidden`, and cross-org user ID `404`. Assert both User reads return decimal-string `securityVersion` and no per-user mapper calls by implementing list SQL as one page-ID query plus one batched detail query, not one query per item. In `SecurityConfigurationTest`, assert each of the four exact GET/path pairs requires authentication, an unsupported method on each same path is denied, and final denyAll remains effective.
...
- [ ] **Step 3: Implement minimal reads.** `IamAdminMapper` selects page IDs/count with identical `org_id`/filters, then fetches users, organization-member data, assignments, and durable `app_user.security_version` for all page IDs in batched queries; service assembles the frozen User representation with decimal-string `securityVersion`. Catalog queries read V3 Roles/Permissions. Add only exact `HttpMethod.GET` plus exact path matchers for the four completed routes; no route-family wildcard; final denyAll remains.
...
- User status request/response contract: `UpdateUserStatusRequest(status, expectedVersion)` requires a decimal-string `expectedVersion`; every successful User representation returns decimal-string `securityVersion`, both mapped to durable `app_user.security_version`.
...
**User-status RED details:** In the Step 1 suite add `statusChangeBumpsVersionAndAudits`, `staleExpectedVersionIsConflict`, `staleSameStatusIsConflict`, and `matchingSameStatusIsNoOp`; assert decimal-string request `expectedVersion`/response `securityVersion`, exact one-time increment, and no mutation/audit on conflict or matching same-status no-op. In `SecurityConfigurationTest`, assert the three exact method/path pairs require authentication, unsupported methods on the same paths are denied, and final denyAll remains effective.
...
- [ ] **Step 3: Implement minimal transactions.** Annotate application use cases `@Transactional`. For user status, inside the transaction perform fresh actor `USER_MANAGE` authorization, current-organization target lookup/lock, locked `security_version` comparison, then status evaluation in that order. Any version mismatch returns `409 STATE_CONFLICT` with no mutation/audit, even for same status. Matching version plus real change updates status, increments version exactly once, writes audit, and registers after-commit invalidation; matching version plus same status returns current User without bump/audit. Role assignment/revoke lock actor/target rows, validate Role before typed resource, map invisible resources to 404, insert/delete current grant, append audit, then call `bumpInTransaction`. Add only the three exact method/path authenticated matchers; no family wildcard; final denyAll remains.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated `POST /api/v1/invitations`, deny an unsupported method on the same path, preserve the separately public invitation-accept operation, and prove final denyAll.
...
- [ ] **Step 3: Implement minimal creation.** Use `SecureRandom` 32 bytes and URL-without-padding encoding; insert PENDING invitation and audit in one transaction, call delivery before commit, never return/log token. Configure `aicostops.iam.invitation-default-lifetime:72h`, max `168h`, dev mailbox path and invite URL. Default bean throws `503 DEPENDENCY_TEMPORARILY_UNAVAILABLE`; open only exact authenticated `HttpMethod.POST /invitations` while public token acceptance stays public; no family wildcard; final denyAll remains.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated GET/POST `/api/v1/projects` and PATCH `/api/v1/projects/{id}`, deny unsupported methods on those same paths, and prove final denyAll.
...
- [ ] **Step 3: Implement minimal explicit SQL.** `ProjectMapper.countAuthorized` and `findAuthorizedPage` both require `org_id`, status filter, and either ORG flag or `id IN allowedProjectIds`; never omit the scope predicate for an empty set. POST forces current `org_id` and ACTIVE. PATCH updates only name/status after scoped lookup. Open only the three exact `HttpMethod`/path project operations as authenticated; no `/projects/**`; final denyAll remains.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated GET/POST member-collection and DELETE member-row paths, deny unsupported methods on each same path, and prove final denyAll.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated GET/POST `/api/v1/teams` and PATCH `/api/v1/teams/{id}`, deny unsupported methods on those same paths, and prove final denyAll.
...
- [ ] **Step 3: Implement minimal team SQL/service/controller.** Mirror the frozen authorization contract, not project code by copy-pasted Role logic; `TeamMapper` receives computed `ResourceScope`, count/rows share predicates, and only the three exact `HttpMethod`/path team operations become authenticated; no `/teams/**`; final denyAll remains.
...
- [ ] **Step 3: Implement minimal lifecycle.** Use transaction/locking and status transitions identical in semantics to Task 9 but through independent team mapper SQL; never infer project access from team membership. Open only the exact GET/POST/DELETE `HttpMethod`/path member operations; no `/teams/**`; final denyAll remains.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated GET/POST `/api/v1/cost-centers` and PATCH `/api/v1/cost-centers/{id}`, deny unsupported methods on those same paths, and prove final denyAll.
...
- [ ] **Step 3: Implement minimal explicit SQL.** Pass computed scope to count/page/update lookup; do not use `organization_member.default_cost_center_id` as implicit authorization. Open only the three exact `HttpMethod`/path cost-center operations; no family wildcard; final denyAll remains.
...
**Route regression requirement:** In `SecurityConfigurationTest`, assert exact authenticated GET/POST `/api/v1/provider-accounts` and PATCH `/api/v1/provider-accounts/{id}`, deny unsupported methods on those same paths, and prove final denyAll.
...
- [ ] **Step 3: Implement minimal provider contract.** Use existing columns only; validate metadata keys before JSON serialization; apply current-org predicate to count/rows/detail/update; open only the three exact `HttpMethod`/path provider-account operations; no family wildcard; final denyAll remains.
...
### Task 14: Cumulative security route characterization and API authorization regression

**Files:**
- Modify only if characterization finds a defect: `backend/src/main/java/com/aicostops/iam/infrastructure/SecurityConfiguration.java`
- Modify: `backend/src/test/java/com/aicostops/shared/security/SecurityConfigurationTest.java`
...
- Produces: a cumulative characterization/regression gate for the exact method-specific transport access already added in Tasks 5–13 while application services enforce permission/scope; production configuration changes are permitted only if characterization finds a concrete defect.

- [ ] **Step 1: Write characterization security tests.** Assert every approved method/path pair anonymously returns 401 and authenticated traffic reaches its controller; unsupported methods such as `POST /users` and `DELETE /projects/{id}`, plus all `/api/v1/evidence`, `/costs/charges`, `/budgets`, `/ledger` routes, remain denied; final `anyRequest().denyAll()` is present; EMPLOYEE project create returns 403; scoped wrong-resource PATCH returns privacy 404.
- [ ] **Step 2: Run characterization.** Run `Push-Location backend; .\mvnw.cmd -Dtest=SecurityConfigurationTest -Dit.test=M1AuthorizationApiIntegrationTest verify; Pop-Location`. Expected: initial GREEN is valid and preferred because Tasks 5–13 already added exact matchers. A failure identifies a concrete regression; do not assume or manufacture an earlier broad-matcher bug.
- [ ] **Step 3: Remediate only an observed defect.** If characterization fails, make the smallest change to restore exact `HttpMethod` plus exact path matching, public invitation-accept ordering, and final `anyRequest().denyAll()`. Never add `/users/**`, `/projects/**`, `/teams/**`, or another broad family matcher. If already GREEN, leave production `SecurityConfiguration` unchanged.
- [ ] **Step 4: Confirm GREEN.** Re-run targeted command. Expected: route exposure and backend 403/404 tests pass; a test-only Task 14 diff is acceptable.
- [ ] **Step 5: Refactor tests only when useful.** Deduplicate cumulative test fixtures without weakening exact operation assertions or final denyAll coverage.
...
- [ ] **Step 8: Commit.** Stage only the changed security characterization/remediation files and commit `test(security): lock down M1 route exposure`.
...
- Parser baseline: the verified backend dependency tree already contains `org.yaml:snakeyaml:2.6` through Spring Boot. Use that existing parser in the test; if the implementation baseline later lacks it, add only the minimum test-scope YAML parser dependency, preferring a Spring Boot-managed version.

- [ ] **Step 1: Write RED parsed-contract tests.** Parse `openapi.yaml` with SnakeYAML into mappings/lists and traverse the model; text contains/grep assertions are not correctness proof. Implement exactly `openApiParses`, `openApiContainsExactlyApprovedM1Operations`, `pagedContractsUseExistingPageShape`, `catalogContractsAreUnpaged`, `idsAndVersionsAreSerializedAsStrings`, `problemResponsesUseProblemSchema`, and `eachM1OperationDeclaresExactResponseStatuses`. Build the approved operation inventory and an operation-to-exact-response-status-set map, then assert equality per operation rather than checking one generic union of statuses.
...
- [ ] **Step 3: Update documents minimally.** Replace generic objects for implemented endpoints; add User/Role/Permission/assignment/invitation/project/team/member/cost-center/provider schemas. Preserve and implement the frozen User status `expectedVersion` contract and response `securityVersion`; document both as decimal strings, status-state/concurrency behavior, privacy 404, invalid Role/scope 400, conflict 409, and delivery 503. Synchronize the API matrix, error catalog, and OpenAPI in the same task.
- [ ] **Step 4: Run GREEN.** Re-run contract test. Expected: YAML parses; the operation set and every per-operation response-status set match exactly; page/catalog/string ID-version/Problem schema assertions pass.
...
- [ ] **Step 6: Run cross-document regression.** Use targeted search only to inspect matrix/error-catalog/OpenAPI synchronization, then rerun `Push-Location backend; .\mvnw.cmd -Dtest=M1OpenApiContractTest test; Pop-Location`. Expected: the parsed executable contract—not search output—proves exact routes, schemas, and response statuses.
...
- [ ] **Step 1: Write RED UI tests.** Users: loading, empty, ProblemDetail, page data with string `securityVersion`, USER_READ without USER_MANAGE hides status action, USER_INVITE and ROLE_ASSIGN independently gate actions, mutation pending, `userStatusSendsExpectedVersion`, `userStatusConflictRefetchesWithoutRetry`, and successful mutation invalidates `settingsKeys.users` and auth-me. A 409 displays the ProblemDetail, invalidates/refetches User list/detail, and executes the mutation exactly once. Roles: loading, empty, error, unpaged catalog and read-only scope display.
...
- [ ] **Step 3: Implement minimal typed client/UI.** `settingsApi.listUsers({page,size})`, `getUser`, `updateUserStatus(id,{status,expectedVersion})`, `listRoles`, `listPermissions`, `create/revokeRoleAssignment`, `createInvitation`; pass the displayed User's `securityVersion` unchanged as `expectedVersion`, keep both typed as strings, and on 409 show the conflict plus invalidate/refetch User queries without blind retry. Use focused Ant tables/forms/drawers and `toProblemDetail`, not a monolithic page file.
...
- [ ] **Step 1: Write RED smoke assertions.** Script must register publicly, identify that member, bootstrap/login the first administrator as bounded below, then use HTTP to assert `/auth/me.permissions`, create an invitation, assign/revoke Roles, change user status with `expectedVersion`, create/read/update each master record, manage project/team members, verify EMPLOYEE create 403, verify a wrong scope created through a real scoped assignment returns 404, revoke a real assignment and prove the old JWT returns 401, and confirm an M2 URL remains denied. Acceptance evidence must also cite the HTTP-driven integration assertions that verify audit rows for API-created subjects and secret absence; do not add a direct SQL audit query to the smoke.
...
- [ ] **Step 3: Finalize deterministic dev setup and evidence.** Reuse compose MySQL/Redis/backend/frontend and existing public registration: register a unique EMPLOYEE, identify its organization member, use `docker compose exec -T mysql mysql ...` only to insert one ORG-scoped SYSTEM_ADMIN assignment for that member and increment its `security_version`, then log in again as the admin fixture. From that login onward, direct SQL is forbidden for acceptance behavior: invitation, Role assignment/revoke, master-data, memberships, user status, wrong-Role/wrong-scope setup and checks, old-JWT invalidation, and audit subject creation all go through real HTTP APIs and API-returned identifiers. Add only required dev invitation-mailbox mounts, not new services. Evidence maps exact commands/tests to #22/#23/#25 and contains exactly: “Direct SQL was used only to bootstrap the first test administrator, not to satisfy #22/#23/#25 acceptance behavior.”
...
- [ ] **Step 7: Check the diff.** Run `git diff --check`; inspect the smoke script for every `mysql` invocation and confirm its only direct SQL invocation is the first-administrator bootstrap. Expected: exit 0, audit evidence comes from HTTP-driven tests over API-created data, evidence contains the exact bootstrap-boundary sentence, and no invitation/JWT/password secret is recorded.
