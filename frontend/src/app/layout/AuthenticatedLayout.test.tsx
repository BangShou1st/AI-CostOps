...
describe('AuthenticatedLayout desktop sidebar', () => {
  ...
  it('hidesNavigationWithoutReadPermission', () => {
    ...
    expect(screen.queryByRole('menuitem', { name: '服务身份' })).not.toBeInTheDocument()
    expect(screen.queryByRole('menuitem', { name: '网关凭证' })).not.toBeInTheDocument()
    expect(screen.queryByRole('menuitem', { name: '模型与定价' })).not.toBeInTheDocument()
  })
