...
class AdvisorInferenceWorkerTest {
    ...
    void happyPathLinksDispatchAndCompletes() {
        ...
        when(jobs.findActiveProfile(7L)).thenReturn(profileRow());
        when(jobs.findInternalCredential(7L)).thenReturn(credentialRow());
        when(jobs.findLogicalModelOf(21L)).thenReturn(9L);
        when(jobs.findAnomaly(eq(7L), eq(3L))).thenReturn(new AdvisorJobMapper.AnomalySubject(
                "ORGANIZATION", "org:7", "USD", new java.math.BigDecimal("30.00"),
                new java.math.BigDecimal("10.00"), new java.math.BigDecimal("20.00"),
                new java.math.BigDecimal("200.0000"), 10.0, "[]"));
    ...
    private static AdvisorJobMapper.JobRow jobRow() {
        return new AdvisorJobMapper.JobRow(100L, 7L, 8L, "ANOMALY", 3L, "fp", "[]", 1, "PENDING",
                null, null, null, 1, null, Instant.now(), null, null);
    }
    ...
    private static AdvisorJobMapper.JobRow stuckJob(long jobId, long requestId, String state) {
        return new AdvisorJobMapper.JobRow(jobId, 7L, 8L, "ANOMALY", 3L, "fp", "[]", 1, "CLAIMED",
