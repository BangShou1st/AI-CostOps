package com.aicostops.gateway.provider.shared;
...
import com.aicostops.gateway.provider.openai.OpenAiSseDecoder;
import com.aicostops.gateway.provider.openai.OpenAiWireDtos;
...
/**
 * Shared OpenAI-compatible Chat Completions transport (M18 V3).
 *
 * <p>Endpoint authority is the frozen {@link ProviderCallContext} (resolved
 * from the ACTIVE connection profile), never the provider catalog default.
 * Adapters never follow redirects; authentication headers are built only
 * from server-governed context. Subclasses own proxy policy and User-Agent.
 */
public abstract class AbstractOpenAiCompatibleAdapter implements ProviderChatAdapter {
    ...
    protected final DispatchEndpointGuard endpointGuard;
    ...
    /** Server-owned User-Agent for this adapter; null means no override. */
    protected abstract String userAgent();

    protected abstract void validateCredentials(ProviderCallContext context);

    protected void authHeaders(WebClient.RequestBodySpec spec, ProviderCallContext context) {
        var secret = new String(context.providerSecret(), StandardCharsets.UTF_8);
        if ("API_KEY".equals(context.credentialType())) {
            var headerName = context.authHeaderName() == null ? "X-Api-Key" : context.authHeaderName();
            spec.header(headerName, secret);
            return;
        }
    ...
    private void applyHeaders(org.springframework.http.HttpHeaders headers, ProviderCallContext context) {
        if ("NONE".equals(context.credentialType())) {
            headers.add("X-Client-Request-Id", boundedRouteDecisionId(context.routeDecisionId()));
            if (userAgent() != null) {
        ...
        var secret = new String(context.providerSecret(), StandardCharsets.UTF_8);
        if ("API_KEY".equals(context.credentialType())) {
            headers.add(context.authHeaderName() == null ? "X-Api-Key" : context.authHeaderName(), secret);
        } else {
    ...
    static String targetUrl(ProviderCallContext context) {
        var base = context.baseUrl().endsWith("/")
                ? context.baseUrl().substring(0, context.baseUrl().length() - 1) : context.baseUrl();
        var path = context.completionPath() == null ? "/chat/completions" : context.completionPath();
        return base + (path.startsWith("/") ? path : "/" + path);
    }

    private Object wireRequest(ProviderCallContext context, ChatCompletionCommand command, boolean stream) {
    ...
    static ProviderExecutionException mapTransportError(Throwable ex) {
