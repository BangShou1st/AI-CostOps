...
import com.aicostops.budget.application.CommitmentIdempotencyStore;
...
/**
 * Commitment reservation/replay against {@code api_idempotency}. The raw key
 * is never stored: the natural key keeps {@code hex(SHA-256(exact UTF-8
 * key))}, so distinct caller keys never conflate and the table's collation
 * cannot merge them either. Identical semantics to the expense idempotency
 * store, on the same shared table.
 */
@Component
public class MyBatisCommitmentIdempotencyStore implements CommitmentIdempotencyStore {

    private final CommitmentIdempotencyMapper mapper;
    private final Clock clock;

    public MyBatisCommitmentIdempotencyStore(CommitmentIdempotencyMapper mapper, Clock clock) {
        this.mapper = mapper;
        this.clock = clock;
    }
