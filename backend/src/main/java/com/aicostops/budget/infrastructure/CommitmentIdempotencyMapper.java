...
/**
 * Persistence for commitment command idempotency against the existing
 * {@code api_idempotency} table (shared with every other command family).
 * The natural key stores the 64-char SHA-256 fingerprint of the exact raw
 * Idempotency-Key header, never the raw key.
 */
@Mapper
public interface CommitmentIdempotencyMapper {
