...
public interface LedgerPostingMapper {
    ...
    String POSTING_COLUMNS = """
            ...
            lp.posting_actor_type,lp.posted_by_member_id,lp.posted_at,lp.created_at
            """;
    ...
    @Insert("""
            ...
            VALUES (#{organizationId},#{postingKey},'GATEWAY_SETTLEMENT',#{settlementId},NULL,
                    #{billingPeriodId},'POSTED','SYSTEM',NULL,#{postedAt},#{createdAt})
            """)
    int insertSystemGatewaySettlementPosting(
            @Param("organizationId") long organizationId,
            @Param("postingKey") String postingKey,
    ...
    @Insert("""
            INSERT INTO ledger_entry(
                ...
                allocation_line_id,correction_group_id,reverses_entry_id,created_at)
            VALUES (#{organizationId},#{postingId},#{entryIndex},#{entryType},#{amount},#{currency},
                    #{projectId},#{costCenterId},#{teamId},#{budgetId},
                    NULL,NULL,#{sourceGatewaySettlementId},NULL,NULL,NULL,#{createdAt})
            """)
    int insertGatewaySettlementEntry(
            @Param("organizationId") long organizationId,
            @Param("postingId") long postingId,
            @Param("entryIndex") int entryIndex,
            @Param("entryType") String entryType,
