...
import com.aicostops.cost.review.application.DuplicateReviewIdempotencyStore;
...
/**
 * Duplicate review reservation/replay against {@code api_idempotency}. The raw
 * key is never stored: the natural key keeps {@code hex(SHA-256(exact UTF-8
 * key))}, so distinct caller keys (including whitespace differences) never
 * conflate and the table's collation cannot merge them either.
 */
@Component
public class MyBatisDuplicateReviewIdempotencyStore implements DuplicateReviewIdempotencyStore {

    private final DuplicateReviewIdempotencyMapper mapper;
    private final Clock clock;

    public MyBatisDuplicateReviewIdempotencyStore(DuplicateReviewIdempotencyMapper mapper, Clock clock) {
        this.mapper = mapper;
        this.clock = clock;
    }
