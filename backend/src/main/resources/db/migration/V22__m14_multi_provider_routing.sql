...
CREATE TABLE routing_policy (
    ...
    active_scope_slot TINYINT GENERATED ALWAYS AS (
    ...
    CONSTRAINT uq_routing_policy_active_scope
        UNIQUE (org_id, project_scope_key, model_id, active_scope_slot),
...
ALTER TABLE gateway_route_attempt
    ADD COLUMN routing_policy_candidate_id BIGINT NULL AFTER routing_policy_id,
    ADD COLUMN routing_policy_version INT NULL AFTER routing_policy_candidate_id,
    ADD CONSTRAINT fk_gateway_route_attempt_policy_org
        FOREIGN KEY (routing_policy_id, org_id) REFERENCES routing_policy (id, org_id),
    ADD CONSTRAINT fk_gateway_route_attempt_candidate_org
        FOREIGN KEY (routing_policy_candidate_id, org_id)
        REFERENCES routing_policy_candidate (id, org_id),
    ADD KEY idx_gateway_route_attempt_policy (org_id, routing_policy_id, routing_policy_version);
